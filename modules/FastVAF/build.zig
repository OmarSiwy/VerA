...
pub fn build(b: *std.Build) void {
    ...
    // Verilog-A conformance
    const conf_va_mod = b.createModule(.{
        .root_source_file = b.path("tests/conformance_va_runner.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const conf_v_mod = b.createModule(.{
        .root_source_file = b.path("tests/conformance_v_runner.zig"),
        .target = target,
        .optimize = optimize,
    });
    conf_v_mod.addImport("zvf", zvf_mod);
    conf_v_mod.addOptions("conf_opts", conf_opts);
    const run_conf_v = &b.addRunArtifact(b.addTest(.{ .root_module = conf_v_mod })).step;
    conf_step.dependOn(run_conf_v);
}
