...
const PpError = Allocator.Error || error{ UndefinedMacro, IfdefTooDeep, IncludeNotFound, MacroRecursion, UnclosedComment };

fn processSource(self: *Preprocessor, source: []const u8) PpError!void {
    const buf = try self.allocator.dupe(u8, source);
    defer self.allocator.free(buf);
    stripComments(buf) catch return error.UnclosedComment;
    try self.processStripped(buf);
}

/// Blank out `//` and `/* */` comments in-place (quote-aware), preserving
/// newlines so diagnostics keep their line numbers.
fn stripComments(out: []u8) error{UnclosedComment}!void {
    var i: usize = 0;
    while (i < out.len) {
        switch (out[i]) {
            '"' => {
...
// ponytail: compile-time hash maps replace sequential mem.eql chains
const directive_map = std.StaticStringMap(void).initComptime(.{
    .{ "define", {} },
    .{ "undef", {} },
    .{ "ifdef", {} },
    .{ "ifndef", {} },
...
const ignored_directive_map = std.StaticStringMap(void).initComptime(.{
    .{ "default_discipline", {} },
    .{ "default_transition", {} },
    .{ "default_nettype", {} },
    .{ "begin_keywords", {} },
...
fn isDirectiveText(text: []const u8) bool {
    ...
    return directive_map.has(name) or ignored_directive_map.has(name);
}
...
fn isIgnoredDirective(directive_text: []const u8) bool {
    ...
    return ignored_directive_map.has(directive_text[0..name_end]);
}
