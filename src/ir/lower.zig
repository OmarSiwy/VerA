...
///
/// `noiseKindsOf` walks a contributed expression, so it sees the generator only
...
/// generator, which is a PSD a host cannot know it should have asked for. Real
/// dataflow here is the same work as giving §4.6.4.6 a `source` identity in
/// `contract.NoiseGen`, and the two want to land together.
var_noise: std.StringHashMapUnmanaged(NoiseKinds) = .empty,
...
fn lowerAssign(self: *Lower, target: Ast.ExprId, value: Ast.ExprId) Oom!void {
    ...
    if (ex.tag(target) == .ident) {
        const k = self.noiseKindsOf(value);
        if (k.count() != 0) {
            const g = try self.var_noise.getOrPut(self.arena, self.file.str(ex.strOf(target)));
            g.value_ptr.* = if (g.found_existing) g.value_ptr.unionWith(k) else k;
        }
    }
}
...
fn noiseKindsOf(self: *const Lower, e: Ast.ExprId) NoiseKinds {
    ...
    switch (ex.tag(e)) {
        ...
        // the identifier and the generator is never exported.
        .ident => return self.var_noise.get(self.file.str(ex.strOf(e))) orelse .{},
