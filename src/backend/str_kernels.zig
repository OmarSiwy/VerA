...
// the upgrade the day a host evaluates one device's units concurrently — this
// file is compiled INTO the device, so that day is the host's to pick, not
// ours, and nothing in this repo can bring it forward. An overrun formats to
