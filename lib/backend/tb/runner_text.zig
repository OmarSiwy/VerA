...
pub const mixed_body =
    ...
    \\    var dig = sim.digital.elaborate(arena, mixed_source, .{ .mixed = .{ .top = mixed_top, .timescale = mixed_timescale, .inserts = &mixed_inserts, .reads = &mixed_reads } }, &bag, &dout.writer) catch |e| mixedFail(&bag, e);
