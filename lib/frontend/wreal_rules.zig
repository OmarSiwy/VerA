...
//!
//! In: a parsed `Ast.SourceFile`. Out: E0918/E0919 in the bag, nothing else.
//! Checked on the parsed file, before elaboration, so a file that breaks one
//! hears the LRM's reason — on BOTH routes: `lib/root.zig` (the analog device
//! compile) and `src/sim/digital` (the digital runner) call this one owner.
//!
//! LRM clauses this file's code cites: §3.7, §6.5.3.
//!
//! Moved from `src/sim/digital/root.zig`, where only the digital runner could
//! reach it: the analog route accepted a wreal port joined to a `wand`.

const std = @import("std");
const Ast = @import("ast.zig");
const diag = @import("diag");

/// §6.5.3 "There can be a maximum of one driver of a real-valued net", and
/// §3.7 "Compatible interconnect are nets of type wire, tri, and wreal ...
...
// elaborated net, which the digital runner's `declare` builds after this.
