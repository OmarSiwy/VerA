...
206 `.va` fixtures: 155 execute, 51 expect rejection, and 0 are marked xfail. (The
number was 208 until `061`/`062` were deleted; the a05/d09/s01 fixtures
arriving in this folder from the table-model and format work are theirs to
count, and this line does not.)
...
| `s9.22.3` | `$driver_state` | `109_driver_state_connectmodule_rejected.va` (+ `31`) — green, E0818 |
| `s9.22.4` | `$driver_strength` | `110_driver_strength_connectmodule_rejected.va` (+ `31`) — green, E0818 |
...
| `s9.22.6` | receiver net resolution via `assign d_receivers = d_drivers;` | — no fixture. The connectmodule with a digital port parses now; the continuous `assign` does not (E0201, and `annex_c_analog_subset/23_continuous_assign_rejected.va` is green on it) |
| `s9.22.7` | the Figure 9-4 worked connectmodule | — no fixture; the whole example is mixed-signal |
...
does not exist in the 2.4 numbering; and described `$param_given` as comparing a value against its
