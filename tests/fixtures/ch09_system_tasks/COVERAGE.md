...
| `s9.4.6` | no display output except `$debug` unless the iteration is accepted | `s01_07_strobe_writes_once_per_accepted_solution.va` — a solve of many Newton iterations whose `$fstrobe` leaves one record per accepted solution. The "shall not" binds the tool, so no source can break it: `CLAUSES.tsv` classifies it `no-prohibition` |
...
| `s9.5.1.2` | analog/digital descriptor sharing | `194_file_descriptor_shared_across_contexts.va` — **passes** (2026-09-24). The smallest design that needs the permission: an `initial` block (the digital context, §7.2.2) opens a file for writing and the `analog` block writes through the descriptor it was handed. The `initial` block runs on the digital kernel because it calls the §9.5 family, and the simulation has one descriptor table (the device's, `contract.FileIo`), so `$fgets` reads back the two bytes `42` |
...
| `s9.5.9` | file position rolled back on a rejected iteration; `$fdebug` excepted | `220_rejected_iterations_leave_no_file_side_effects.va` (exactly k lines after point k however many Newton iterations ran) and `s01_07`. Every §9.5 call is sequenced in the per-accepted-point phase (`codegen.Gen.emitting_display`), so a rejected iteration writes nothing. The duties are the tool's: `CLAUSES.tsv` classifies it `no-prohibition` |
