...
| R1 | ~~`90_dumpfile_twice_rejected.v`~~ **withdrawn 2026-09-23, file deleted** | ~~1364 §18.1 — at most one `$dumpfile` per simulation~~ | No such rule. IEEE 1364-2005 §18.1.1 (printed 325–326) says only that `$dumpfile` "shall be used to specify the name of the VCD file" and that the filename defaults to `dump.vcd`; §18 contains the word "once" nowhere. The quoted "shall be invoked only once, before any `$dumpvars`" is not in the standard, so the refusal tested an invented rule. The claim moves to no row; what a second `$dumpfile` does is unspecified. `$dumpfile`'s real obligation (name the file) stays with 11 |
