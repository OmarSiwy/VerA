...
| `s7-3-2` | four features that carry x/z into the analog context | POSITIVE: `xz_four_state_input_compared.va` (green) reads a digital `reg` through `===`, `!==` and a `case` with `1'bx`/`1'bz` labels as it goes 1, x, z, 0, and every state gives its own value. The clause's own a2d examples, `m01_02` and `m01_03`, are green: their x arm, `avar = avar`, reads the value §3.2 retention holds from the previous evaluation. Also positive, each one operator/statement over a `reg` going 1, x, z, 0: `x_case_equality_compared.va` (`=== 1'bx`), `z_case_inequality_compared.va` (`!== 1'bz`), `xz_case_statement_compared.va` (`case` with x/z labels), `xz_casex_statement_compared.va` and `xz_casez_statement_compared.va` (IEEE 1364 §9.5.1 don't-cares on either side) — all five were `_unsupported` E0130/E0209 rejections on the Verilog-A subset's C.3/C.5/C.7 and are inverted. The rejects below are an error in AMS too: `x_literal_unsupported.va`, `z_literal_unsupported.va`, `xz_contribution_rejected.va` and `xz_ordinary_equality_rejected.va` reproduce the four lines the clause's own `converter` example annotates `// error`; all four land on E0130, "x/z digit in a number literal", on the literal itself. `case_equality.va` is the operator alone on two-state integers, ACCEPTED and asserted (it used to pin E0323 on the Verilog-A subset's C.5; the list above makes `===`/`!==` full-AMS analog features). |
...
| `s7-3-3` | continuous nets probed from a discrete context; interpolation | `m01_08_analog_probe_in_a_digital_expression.va` (green): `always @(posedge clk) out = V(in)` reads 1.0 at tick 10 and 3.0 at tick 30. `digital_probe_unsupported.va` (E0437 on the clause's `sampler` shape) is withdrawn to it |
...
| `s7-3-5` | continuous events detected in a discrete context (Syntax 7-3) | `m01_09_cross_event_in_a_digital_always.va` (green): `always @(cross(V(clk) - 2.5, 1))` counts the rising crossing once and ignores the falling one. `digital_cross_unsupported.va` (E0437 on the same shape) is withdrawn to it |
| `s7-3-6` | parent: synchronization across the digital tick | — host/kernel property, and 8.2 is where the algorithm lives |
| `s7-3-6-1` | analog event in a digital event control, scheduled at the nearest tick | `m01_09`, and ch8's `m02_04` (5.2 ns -> tick 5, 7.6 ns -> tick 8), `m02_07`/`m02_08` (two crossings 0.2 ns apart, both at tick 5, in order), `m02_11` (delivered once), all green. `src/sim/mixed.zig` pins the rounding against a fake analog |
...
| `s7-3-6-3` | analog primary in a digital expression | `m01_08` (green) and `m02_11` (green: `sampled = V(vi)` in the A2D's process reads 0.5, the ramp at the promoted tick 5.0 ns, not 0.52 at the 5.2 ns event). `digital_probe_unsupported.va` and `cross_domain_function_unsupported.va` (E0437) are withdrawn to m01_08 |
| `s7-3-6-4` | analog event variables driving a continuous assign | `analog_event_variable_drives_continuous_assign.va` (wire target) and `analog_event_variable_drives_wreal_assign.va` (wreal target; was `continuous_assign_unsupported.va`, an Annex C.4/C.7 subset refusal withdrawn once `wreal` was accepted). Both xfail on the same gap: the digital engine cannot see an analog variable (E1100) |
...
| `s7-4-4-3` | coercion: a declared interconnect discipline wins unless `resolveto` overrides | `lrm_7_4_4_3.va` cites this id and is green (MIX-LOCAL-001): `ch7443_alt shared;` is a DECLARED interconnect, the parent reads it as `Va(shared)` = 0.5, and the child reaching it through an `electrical` port reads the same node through its OWN declaration, `V(port)` = 0.5 (§4.4: the access name matches the declaration of the port given; §7.4 resolves undeclared nets only). An undeclared net reads `V(plain)` = 0.25 through the discipline resolved from its port. `audit_child_wrong_{potential,flow}_rejected.va` pin the converse: `Va(port)`/`Ia(port)` on the electrical child is E0501. The clause's second half ("unless the `resolveto` connect statement overrides the discipline") is NOT reached: it needs connect-module insertion, and the `m03_` family is where that is measured. The fixture also records, in its header, why the harness could not bias the net: `//!` names accept only `V()`/`I()`, so a user-discipline accessor in a directive generates a testbench that does not build, and the node is fixed by the model instead. `resolution_connect_accepted.va` remains the statement that coercing a declared discipline by `resolveto` is unimplemented |
...
| `s7-8` | automatic insertion at mixed ports | — post-elaboration; no fixture. Its converse, §7.1's MANUAL insertion ("can be manually inserted (by the user)"), is `connect_module_manually_inserted.va`, which passes: the hand-placed bridge is inlined like any child and its digital half runs (E0913 and E0920 are retired) |
...
- A four-state comparison over a discrete net with NO digital driver in the
  module (the LRM's own `a2d` prints `input dnet; wire dnet;` undriven) still
  lands on E0130: only a mixed module (`lower_context.isMixed`) is the
  four-state context, and an undriven top-level discrete input port has no
  kernel value. The `*_compared.va` fixtures add a driver.
