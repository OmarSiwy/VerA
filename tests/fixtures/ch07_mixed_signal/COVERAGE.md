...
Forty-seven `.va` files: eleven run and assert (two of them are xfail), two are
`//! xfail`, thirty-six are plain `//! reject`. Those three numbers are grep-MEASURED (`grep -l '^//!
xfail' *.va` and the same for `reject`), not carried over — the sentence that
stood here said ten xfail, which several waves' closures had falsified. The
...
| `s7-3-7` | no digital function from analog, no analog function from digital | one of the two sentences is checked: `analog_function_from_digital_unsupported.va` now also raises E0430, the §4.7.3/§7.3.7 calling-context rule, at the call in its `initial` block (it pins E0205, which still fires on the block as an item, so its verdict is unchanged). The other half is still masked — `digital_function_from_analog_unsupported.va` is E0205 on the non-analog `function` declaration, so no caller is analysed |
...
TWO fixtures in this folder are `//! xfail`, grep-measured: `discrete_bus_narrow_unsupported.va`
and `discrete_bus_31_unsupported.va`. Six rows have closed and are kept below, because four
of them closed in the way this section said they would — flat single-module defects that
needed no new feature — and because each names a rule that must not silently regress.
...
| `discrete_bus_narrow_unsupported.va` | `s7-3-1` | NOT a capability gap any more. `reg [7:0] r` is declared (as an integer, Table 7-1's own mapping) and `initial r = 8'hff` parses; what blocks the file is that four green fixtures — `annex_c_analog_subset/13_digital_initial_rejected.va`, `digital_initial_unsupported.va`, `discrete_real_from_analog_unsupported.va`, `ch08_scheduling/analog_digital_initial_order_unsupported.va` — pin `reject E0205` on a constant assignment in an `initial` block, three of them arguing under C.7/C.9 that the refusal *is* conformance. No implementation satisfies this fixture and those four at once; re-verdicting them is an owner decision about whether annex C binds VerA |
| `discrete_bus_31_unsupported.va` | `s7-3-1` | Same contradiction at the legal boundary: 31 ones must read as +2147483647 with the integer's sign bit forced to zero. The WIDTH half is checked — E0222 stays silent at 31 and fires at 32 — so what is left is the mapping, and it is blocked by the same four fixtures and not by anything missing |
