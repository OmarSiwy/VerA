...
| `a-1-9` | `paramset_declaration`, `paramset_item_declaration`, `paramset_statement` | `10_paramset.va` — a full `paramset … endparamset` with a `parameter` item and a `.gain = 2.0 * scale;` override statement, applied by instantiating it. **`//! xfail`** |
...
| `a-2-1-3` | `branch_declaration`, `event_declaration`, `integer_declaration`, `real_declaration`, `net_declaration`, `reg`/`time`/`realtime` | `03_declarations.va` (the `discipline_identifier list_of_net_identifiers` arm of `net_declaration`, plus `integer`, `real`, and a named `branch (p, n) path;`), `23_expression_primaries.va` (branch plus `real array[0:1]`), `05` (`integer`, `real`). `event_declaration` is `17_named_event_trigger.va` — **`//! xfail`**. `reg`, `time`, `realtime` and the eleven digital `net_type` spellings: nothing, and nothing should — see A.2.2.1 |
...
`06` ↔ `28`/`17` (event control that works, nesting that must not, and the named-event form
that does not yet), `08` ↔ `35` (an escaped identifier that names something against one
that cannot).
