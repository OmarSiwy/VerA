...
Sixty-eight `.va` files: **thirty-eight run and assert**, **fifteen carry a `//! reject`
arm**, and **fifteen carry `//! xfail`** (grep-measured over this directory). The xfail
ledger was empty until the A.1.1/A.1.5/A.1.8, A.3, A.5, A.7 and A.8.6 rows were written,
and every row of it is a *missing acceptance* — legal full-AMS source VerA refuses —
never a missing check, so nothing here freezes on a diagnostic that is itself the bug.
The older, closed rows are kept below as the record of which gap closed each one.

Twenty-four of the sixty-eight files were written after the first census (45–68), and
every one of them takes a clause that census had listed as uncovered. Eight are green
(`45`, `48`, `52`, `60`, `61`, `65`, `67`, `68`), one is a `//! reject` (`62`), and
fifteen are `//! xfail` (`46`, `47`, `49`–`51`, `53`–`59`, `63`, `64`, `66`). The
fifteen xfail rows all make the same argument in
their own words and cite §1.1 for it: the complete IEEE Std 1364 Verilog is part of
Verilog-AMS HDL, Annex C.16 does not exempt these keywords from the full language, so a
full-AMS compiler must ACCEPT the source rather than refuse it as out-of-subset. Each
one still carries an assertion, so the day VerA accepts the construct the file XPASSes
and names its own gap.
...
| `a-1-5` | `config_declaration`, `design_statement`, `cell_clause`, `liblist_clause` | `47_configuration_source_text.va` — a whole `config … endconfig`: `design work.annex_a_cfg_top;`, `default liblist annex_a_lib;`, an `instance` clause with `use`, and a `cell` clause with `liblist`, plus the carrier module. `//! xfail`: E0201 "construct is not in the supported subset: `config`". The header is written around the difference between that diagnostic and E1001 "source contains no module declaration" — the `config` keyword is RECOGNISED and refused as a subset, not mistaken for an unknown word |
...
| `a-3-1` | `gate_instantiation`, `n_input_gate_instance`, `pass_switch_instance`, … | `56_gate_instantiation_unsupported.va` — `and nand_g1 (c, a, b);`, one `n_input_gate_instance` with its `n_input_gatetype`, `//! xfail` on E0205 "unsupported module item: found and". The header lists the arms NOT spelled (switch primitives, `buf`/`not`, enable gates, pull gates) and their `a-3-4`/`a-3-2` spellings |
| `a-3-2` | `pulldown_strength`, `pullup_strength` | `57_primitive_strengths_unsupported.va` — `pulldown (strong0) pd (o);`, `//! xfail` on E0205 "unsupported module item: found pulldown". The parenthesised `( strength )` bracket is the whole clause and the only place a `strength0` is written outside `54`'s drive-strength declaration |
| `a-3-3` | `input_terminal`, `output_terminal`, `enable_terminal`, … | `56` (`c`, `a`, `b` as `input_terminal`/`output_terminal`) and `59` (`b`, `a`), both `//! xfail` before any terminal is looked at. The `enable_terminal` and `[ drive_strength ]` forms have no fixture |
...
| `a-5-1` | `udp_declaration`, `udp_ansi_declaration` | `58_udp_declaration_unsupported.va` — `primitive annex_a_udp_pulse (o, a); … endprimitive` as a top-level `description`, `//! xfail` on E0201 "construct is not in the supported subset: `primitive`". The non-ANSI arm is taken; `udp_ansi_declaration` is not spelled |
| `a-5-2` | `udp_port_list`, `udp_output_declaration`, `udp_input_declaration` | `58` — `(o, a)` is the `udp_port_list` and the body's `output o;`/`input a;` are the port declarations. `//! xfail` at the `primitive` keyword, which is the same refusal `a-5-1` records |
| `a-5-3` | `udp_body`, `combinational_entry`, `sequential_entry`, `edge_indicator` | `58` — a `table … endtable` body with two `combinational_entry`s (`0 : 1;`, `1 : 0;`). No `edge_indicator`, so no `sequential_entry`, and the header says the table is never read: VerA stops at `primitive` |
| `a-5-4` | `udp_instantiation`, `udp_instance` | `59_udp_instantiation_unsupported.va` — `annex_a_udp_pulse (b, a);` inside a module, `//! xfail` on E0205 "unsupported module item". The UDP is declared AFTER the module on purpose: the probe recorded in the header shows the diagnostic changes with the declaration order, and the instantiation is the thing being refused |
| `a-6` | heading | parent |
| `a-6-1` | `continuous_assign ::= assign [ drive_strength ] [ delay3 ] list_of_net_assignments ;` | — **no fixture here**. `grep -n '^\s*assign' *.va` is empty. This is the one production the directory leaves to the rest of the suite: `annex_c_analog_subset/23_continuous_assign_rejected.va` cites `//! lrm A.6.1` and pins E0205 on `wire w; assign w = 1'b1;`, and that citation is what the census above counts. Digital continuous assignment; the analog counterpart is A.6.10's `<+`, which is everywhere |
...
| `a-7-1` | `specify_block`, `specify_item` | `63_specify_block_unsupported.va` — one `specify … endspecify` carrying four of the five `specify_item` arms (`specparam`, `path_declaration` twice, `pulsestyle_onevent`), `//! xfail` on E0205 "unsupported module item: found specify". ONE FILE FOR FOUR CLAUSES, and the header states the reason rather than implying four separate measurements: VerA declines the block and never looks inside, so every inner production would report the same code |
...
| `a-7-5` | `system_timing_check` | `64_timing_checks_unsupported.va` — four of the twelve commands, chosen so every optional bracket of the three clauses below is taken at least once, `//! xfail` on the same E0205 at `specify` |
...
is required by §1.1, and VerA refuses it. Each of the fifteen carries an assertion so the
day the gap closes the file XPASSes and names itself rather than decaying into
`unasserted`:

| Fixture | Section | First diagnostic VerA gives |
|---|---|---|
| `46_library_source_text.va` | `a-1-1` | E0201 "construct is not in the supported subset: `library`" |
| `47_configuration_source_text.va` | `a-1-5` | E0201 "construct is not in the supported subset: `config`" |
| `49_specparam_declaration_unsupported.va` | `a-2-1-1` | E0205 "unsupported module item: found specparam" |
| `50_ranged_parameter_unsupported.va` | `a-2-1-1`, `a-2-5` | E0208 "expected an identifier: found `[`" |
| `51_output_variable_type_unsupported.va` | `a-2-1-2`, `a-2-2-1` | E0208 "expected an identifier: found `integer`" |
| `53_variable_initializer_unsupported.va` | `a-2-2-1`, `a-2-3` | E0207 "unexpected token: found `=`" |
| `54_drive_strength_unsupported.va` | `a-2-2-2` | E0208 "expected an identifier: found `(`" |
| `55_net_delay_unsupported.va` | `a-2-2-3` | E0208 "expected an identifier: found `#`" |
| `56_gate_instantiation_unsupported.va` | `a-3-1`, `a-3-3`, `a-3-4` | E0205 "unsupported module item: found and" |
| `57_primitive_strengths_unsupported.va` | `a-3-2` | E0205 "unsupported module item: found pulldown" |
| `58_udp_declaration_unsupported.va` | `a-5-1`, `a-5-2`, `a-5-3` | E0201 "construct is not in the supported subset: `primitive`" |
| `59_udp_instantiation_unsupported.va` | `a-5-4` | E0205 "unsupported module item: found annex_a_udp_pulse" |
| `63_specify_block_unsupported.va` | `a-7-1` … `a-7-4` | E0205 "unsupported module item: found specify" |
| `64_timing_checks_unsupported.va` | `a-7-5` … `a-7-5-3` | E0205 "unsupported module item: found specify" |
| `66_case_equality_in_analog.va` | `a-8-6` | E0323 "case equality is not in the analog subset" (lowering) |

They are three shapes of one problem, and the table separates them because the fix does
not. Nine are an unrecognised CONSTRUCT — `library`/`config`/`primitive` at the top
level, `specparam`/`and`/`pulldown`/`specify`/a UDP instance as a module item — and each
of those needs a real parser arm and, for the digital ones, a digital execution model.
Five are a missing optional part of a declaration VerA otherwise parses — a parameter
range, an `output` variable type, a drive strength, a net delay, an initializer — where
the fix is a production rather than a subsystem. One — `66` — is neither: the operator is in the grammar, in Clause 4 and in
§7.3.2's list of what the analog context supports, and lowering refuses it by name.

Twelve of the fifteen cite §1.1 as the reason they must eventually pass: the complete
...
fifteen `//! xfail` rows above are exactly that coverage, written as xfails instead of
...
is fine for the thirty-eight run fixtures here, which only probe their own biased ports. It is no
...
subset's edge actually is. The `//! xfail` files obey the same rule for the same reason,
one construct each — which is why `63`/`64` are two files for one section and why `56`/
`57` are two files for one family of primitives.
...
declaration brackets, all refused, against the same declarations written without them,
which `52` runs), `54` ↔ `57` (the `strength` bracket on a net declaration and on a pull
gate, two spellings of `strong0`/`pull0`), `58` ↔ `59` (the UDP declaration and its
instantiation, refused by different stages in different words), `56`/`57` ↔ `63`/`64`
(three module-item families whose first diagnostic is the same E0205), `65` ↔ `66`
(`===` refused where §7.3.2 supports it, against the rest of the operator list evaluated),
`46` ↔ `47` (two of A.1.1's three `library_description` arms, refused as one family), and
