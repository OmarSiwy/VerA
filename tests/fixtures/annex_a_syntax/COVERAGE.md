...
Forty-four `.va` files: **thirty run and assert**, **fourteen carry a `//! reject` arm**,
and **none carries `//! xfail`** (grep-measured over this directory). The fourteen-row
xfail ledger this paragraph used to quote was a clean debt ledger and it emptied: every
row was a *missing acceptance* — legal analog-subset source VerA refused or mis-evaluated
— never a missing check, so nothing here froze on an impossible diagnostic. The ledger
below is kept as the record of which gap closed each row.
...
EMPTY — grep finds no `//! xfail` in this directory. The four files that held the last
rows are listed with what closed them, and the six numbered groups below are kept because
each was a prediction this document made about how the fixtures would fall, and each
