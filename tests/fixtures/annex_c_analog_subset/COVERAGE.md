...
| C.4 Data types | Clause 3 applies except: discrete domain binding, `wreal`, `` `default_discipline `` | `08_wreal_accepted.va` (was `08_wreal_rejected.va`; INVERTED with the other bullets: §3.7's wreal is accepted and read by the analog block). Bullets 1, 2 and 3 all had their verdicts INVERTED once the project settled that VerA targets Verilog-AMS and not the subset: `07_discrete_domain_binding_accepted.va` (was `07_..._rejected`, now the §7.2.1 rule) and `09_default_discipline_accepted.va` (was `09_..._rejected`, now §10.2's own binding — the directive is the module's only source of a discipline, so a tool that parses and discards it reaches E0337 and asserts nothing). What survives of bullet 3 is `18_no_discipline_rejected.va` — green, `//! reject discipline`: a module that declares *no* discipline anywhere has no potential to probe under any dialect |
| C.5 Expressions | Clause 4 applies except `===` and `!==` | `10_case_equality_accepted.va` (`===`, was `10_case_equality_rejected.va`), `17_case_inequality.va` (`!==`). Both INVERTED for the reason C.4 and C.7 were: C.5 is the subset's rule and §7.3.2 names both operators as supported in the AMS analog context, so each now asserts the values IEEE 1364 §5.1.8 gives on integer operands. Both operands stay integer so §4.2.1's real-operand rule (`ch04_expressions/29_case_equality.va`, E0369) is not what the file measures |
...
| C.7 Analog behavior | Clause 5 applies except digital behavior/events and `casex`/`casez` | `11_casex_accepted.va` and `12_casez_accepted.va` (were `_rejected`, pinning E0416, now retired): INVERTED, §7.3.2 lists casex/casez among the AMS analog context's features; four-state behaviour is `ch07_mixed_signal/xz_casex_statement_compared.va`/`xz_casez_statement_compared.va`. Bullet 1's `initial` half was INVERTED and has been re-verdicted, for the reason bullets C.4/1 and C.4/3 were: `13_digital_initial_accepted.va` (was `13_..._rejected`) now asserts §7.2.2's own first sentence instead of demanding a diagnostic no conforming AMS compiler may emit. Its `always` half is re-verdicted with the mixed-signal runner (ch07 step 4): `14_digital_always_rejected.va` is withdrawn — its claim moves to `ch07_mixed_signal/m02_10_macro_process_runs_after_nba.va`, an `always` block that runs. Then `21_digital_event_control_rejected.va` (`posedge`, `negedge`), `22_nonblocking_assign_rejected.va` (`<=`), `23_continuous_assign_accepted.va` (was `_rejected`; `assign` runs and the net reads 1), `25_digital_procedural_rejected.va` (`fork`, `join`, `wait`). Positive side: `05_analog_event.va` and `19_named_event_in_subset.va` — the §5.10.4 named event is on the analog side of the C.7 line and now works |
| C.8 Hierarchical structures | Clause 6 applies except real value ports (§6.5.3) | INVERTED: `15_real_value_port_accepted.va` (`input wreal`), `28_real_value_output_port_accepted.va`, `29_real_value_inout_port_accepted.va` (were `_rejected`). C.8 is the subset's carve-out; §6.5.3 makes the port legal in Verilog-AMS, so each file now asserts the real value crossing the boundary in its direction. `annex_a_syntax/11_module_instantiation.va` and `ch07_mixed_signal/hierarchy_unsupported.va` cite C.8 for the hierarchy it keeps |
...
|---|---|---|
| `11_casex_accepted.va` | was `11_casex_rejected.va`, C.7 bullet 2 | First E0209 (parser recovery), then E0416 (the subset rule, now retired); inverted once casex lowered (`lower_expr.caseMatch`) |
| `12_casez_accepted.va` | was `12_casez_rejected.va`, C.7 bullet 2 | As 11, for casez |
...
casex/casez are implemented now and `11`/`12` are positive.
