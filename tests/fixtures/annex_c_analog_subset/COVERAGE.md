...
| `19_named_event_in_subset.va` | C.2 + C.7 / §5.10.4 — a named event triggered and detected inside the analog block is *in* the subset | VerA does not implement named events: `event tick;` is E0203, `-> tick` is E0209, `@(tick)` is E0705 |
...
Note the direction of `19`. Every other xfail here is a rule VerA fails to *enforce*;
`19` is a construct VerA fails to *accept*. It is also the only file in the directory
with no `//! reject` arm whose xfail is still live.
