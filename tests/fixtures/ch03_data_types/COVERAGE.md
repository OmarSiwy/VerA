...
| 3.9 Disciplines of primitives | Both paragraphs, both ways. Analog: `primitive_net_defaults_to_electrical.va` solves a divider over a net only two unattributed `resistor`s touch (electrical by the last sentence). Digital: `digital_primitive_on_mixed_net_rejected.va` is the refusal — a gate terminal on an `electrical` net with no `` `default_discipline `` in force is E0960, the error "during discipline resolution" the clause promises; `ch08_scheduling/switch_primitive_accepted.va` is the legal counterpart (directive plus connect statement). |
