...
Four rows left this table: `16_paramset.va` (a `paramset` declaration is now read past and dropped — §6.4 gives one effect only through an instance, which is still E0204), and `18_multidimensional_array.va`, `24_multidimensional_strings.va`, `77_assignment_pattern_replication.va` (A.2.2.1's `{ dimension }` is a loop now, and lowering scalarizes the whole shape).
