...
| `s10-2` | Nonempty base and qualifier forms are `01_default_discipline.va` and `02_default_discipline_qualified.va`; `24_default_discipline_empty_reset.va` first establishes a default and then uses the empty directive to reset it. VerA consumes these directives but does not apply discipline resolution, and the fixtures explicitly declare their net disciplines, so the snapshots prove directive acceptance rather than the resulting discipline state. |
