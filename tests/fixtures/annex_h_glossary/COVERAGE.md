...
| `glossary-f` | flow | **No fixture asserts a flow.** All nine contribute one, and `05_module_parameter.va` asserts the *expression* `gain*V(p,n)` = 0.625 that its contribution then uses, but `I()` never appears outside the left of a `<+` anywhere in this folder. It was deliberate: `04_branch_flow_potential.va`'s header explains that reading a flow back used to drag the file into the (now closed) antisymmetry gap below, so it is a missing fixture and no longer a blocked one. (The previous revision credited `04` with "flow quantity"; `04` asserts three potentials and no flow.) |
...
## The xfail ledger, which lived elsewhere
...
- **`glossary-f` flow, as a value read back.** Nine contributions, zero
  ...
  probe* under §5.4.2.1. This used to collide with the antisymmetry gap, which is
  closed, so nothing blocks it now: the term is still demonstrated only as the
  left side of a `<+`, and that is now purely a missing fixture.
