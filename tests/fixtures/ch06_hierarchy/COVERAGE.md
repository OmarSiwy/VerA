...
| `s6.4` | paramsets | `paramset_unsupported.va` — **`//! xfail`**: the paramset DECLARATION is accepted now (read past and dropped — §6.4 gives one effect only through an instance), but the instantiation that would select it is E0204 |
| `s6.4.1` | the `.module_parameter_identifier = expr;` assignment form | `paramset_unsupported.va` writes `.k = 2.0 * gain;` — **`//! xfail`**, on the instantiation. No fixture cites this id; credited on content |
| `s6.4.2` | overload selection by parameter range | `paramset_overload_unsupported.va` (two paramsets on one module, `l = 0.5` admitted by exactly one) — **`//! xfail`**, E0204 |
...
| **Missing semantic check — VerA accepts what the LRM forbids.** No diagnostic at all is produced. The worst failure mode in the set: a wrong model compiles clean. | 4 | `duplicate_declaration_rejected.va` (two declarations, one scope), `ansi_port_redeclared_rejected.va` (ANSI port redeclared in the body), `untyped_port_behavioral_use_rejected.va` (`V()` on a discipline-less port), `mfactor_double_scaling_rejected.va` (a model that multiplies its own contribution by `$mfactor`). The six generate rules that used to be in this row — the two block-name collisions, the instance-array collision, the nested region, the non-constant scheme and the `parameter` declaration — now diagnose at E0228/E0229/E0230/E0428 |
