...
- **S01 fixtures 05 and 06** — `§5.10.2 … "the initial_step event is active on the first point of
  ...
  the entire justification for calling W0851 a deviation rather than a correct refusal.
...
- **S01 — §9.4.1 change detection is now pinned by nothing.** Both 05 and 06 fail today because
  W0851 drops the event-guarded `$fopen`, so `fd` is never assigned and every measured value is
  `bytes = -1` — a number that is not a function of whether change detection exists. The row's
  headline clause has zero live coverage; SPEC.md concedes this in one place and still lists both
  fixtures under the §9.4.1 "pins" column.
