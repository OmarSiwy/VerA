...
| 5.6.1.3 Value retention | `value_retention.va` **xfail** — VerA does not implement value retention: it accumulates all three potential contributions to 8.0 and stamps the flow of 2.0 on top, and its testbench forces node voltages rather than solving, so `V(p,n)` reads the bias. This is the clause's own Example 2, answer 7.0; it is the only fixture in the tree that carries the numeric rule |
...
| 5.7 Analog procedural assignments | `procedural_assignments.va` (positive); `assignment_concatenation_invalid.va` (E0317); `assignment_hierarchical_invalid.va` (E0214, the right verdict by the shorter route). `assignment_array_mismatch_invalid.va` **xfail** — VerA does not implement whole-array assignment: `A = B` and `A = C` alike die at E0313 (unknown variable), so the compatible pair is refused with the incompatible one and no size check is ever reached |
...
| 5.10.3.2 above function | `event_above_enable.va`, `event_above_tolerance.va`. `event_above.va` **xfail** — VerA's `above()` is level-triggered, not edge-triggered: codegen emits `expr > 0.0` with nothing compared against the previous point, so it re-fires on every sweep point where the expression is positive and `n` counts points instead of crossings |
...
| 5.10.4 Named events | `named_event_unsupported.va` **xfail** — VerA does not implement named events: `event tick;` is refused at E0203 and `@(tick)` at E0705 |
...
`loop_contribution_invalid.va` (contribution in a runtime loop),
`assignment_array_mismatch_invalid.va` (array size compatibility) and
...
`indirect_contribution.va` / `multiple_indirect.va`, which share one root
cause: the generated testbench forces every unknown to its declared operating
point instead of solving, so no indirect target can move.

That last pair is the one entry here that is a *harness* limitation rather than
a compiler one, and it is worth keeping separate: the two files describe systems
with unique exact solutions in dyadic rationals, so the day the testbench solves
instead of forcing, they turn green without being touched.
