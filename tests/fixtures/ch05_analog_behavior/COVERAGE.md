...
| 5.10.5 Digital events in analog behavior | — **no fixture.** Analog behavior sensitive to `posedge`/`negedge`, state-change and named digital events is entirely uncovered; digital source execution and mixed-signal coordination remain open full-AMS requirements. The standalone scheduler core does not supply this behavior |
