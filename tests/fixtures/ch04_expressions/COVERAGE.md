...
| 4.2.9 Bitwise operators | `56_bitwise_and.va`, `57_bitwise_or.va`, `58_bitwise_xor.va`, `59_bitwise_xnor.va` (both `^~` and `~^`), `62_bitwise_not.va`. `unsigned_operand_makes_the_expression_unsigned.va`: the unsigned-comparison sentence over operands that are themselves expressions (sum, comparison, conditional; IEEE 1364 §5.5.1) |
...
| 4.6.2 DC analysis | `dc_sweep_every_point_is_dc_and_static.va` — `analysis("dc")`/`("static")` are 1 and the other four names 0 at each of three points of one `//! sweep`; `26_analysis.va` is the single-point half. The clause's other duties (carry-over within a sweep, none between sweeps, `"nodeset"` only in the nodeset phase) bind the simulator; `CLAUSES.tsv` classifies the clause `no-prohibition` with the quote. `"nodeset"` appears in no fixture in this directory |
