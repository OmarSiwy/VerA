...
| `16_file_display_diagnostics.va` | The complete open/write/flush/close lifecycle, and since W0851 was lifted the guarded `$fdisplay`/`$warning` pair inside it actually runs — `combined.log` gets its record and the warning reaches the transcript. The pair is still not the ASSERTION (a line that appears on one arm only is indistinguishable from one that never ran), so what this file *asserts* is §4.2.5's integer-valued relational, §9.7.1's `$abstime` and Ohm's law. The lifecycle itself is pinned by `ch09_system_tasks/{07,08,052}`; here it is context. |
