...
| `16_file_display_diagnostics.va` | The source contains the complete open/write/flush/close lifecycle and it now runs — but the `$fdisplay`/`$warning` pair sits under a conditional ON PURPOSE (W0851 drops it, which the header explains), so what this file asserts is §4.2.5's integer-valued relational, §9.7.1's `$abstime` and Ohm's law. The lifecycle itself is pinned by `ch09_system_tasks/{07,08,052}`; here it is context. |
