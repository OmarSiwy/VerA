...
3. **`type_string` ("global"/"instance")**: §9.13.1/.2 make it legal ONLY
   ...
   `lowerRandom`'s blanket reject predates §6.4.2 paramsets landing. DONE
   same day: `rewriteParamsetDist` in elaborate.zig folds the draw at
   elaboration (literal-seed ceiling, `ponytail:` at site); fixtures 175/176.
4. **Correlated noise (§4.6.4.6)**: the spec's own normative example is
   ...
   column exists for). DONE same day: `Lower.NoiseSrc` keyed by declaring call,
   `NoiseGen.source: ?u16`, `//! noise` asserts `kind(row,col)#<source>`;
   fixtures 161 + 38 pin it.
5. **`$random`/`$rdist_*` algorithm**: §9.13.3 pins it normatively — "17.9.3
   ...
   nonconforming. DONE same day: `rng_kernels.zig` rewritten bit-exact to the
   listing (provenance URLs in its header, digits generated by running the
   fetched C, never hand-computed), fixture 171 pins the stream two draws
   deep. §3's prediction held: the file was the single fix site.
